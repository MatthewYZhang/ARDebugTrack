...
class MotionClient(object):
    ...
    def _create_data_socket(self, port):
        ...
        socket_data.setsockopt(socket.IPPROTO_IP, socket.IP_ADD_MEMBERSHIP, socket.inet_aton(self._multicast_ip) + socket.inet_aton(self._local_ip))
        # socket_data.bind(('', port))
        socket_data.bind((self._local_ip, port))
        return socket_data
    ...
    def _data_callback(self, data_socket):
        ...
        try:
            while self._is_running:
                ...
                data, addr = data_socket.recvfrom(SIZE_BUFFER)
                if len(data):
